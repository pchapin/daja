...
class ConfigurationSettings(private val configurableItems: Map[String, String => String]):
  ...
  private def killComments(line: String): String =
    ...
    for currentIndex <- 0 until line.length do
      line(currentIndex) match
        case '\"' => currentState = if (currentState == Normal) Quote else Normal
        case _ =>
          if currentState == Normal && line(currentIndex) == '#' then
      ...
      while line != null do
        ...
        if line.nonEmpty then
          ...
          if fields.length == 2          &&
             ...
             fields(1).length >= 2       &&
             fields(1).charAt(0) == '\"' &&
             fields(1).charAt(fields(1).length - 1) == '\"' then
  ...
  def put(name: String, value: String): Unit =
    if configurableItems.contains(name) then
...
object ConfigurationSettings:
   ...
   * nonexistent file. More powerful validators can be defined that do check these features if
   * desired.
   */
